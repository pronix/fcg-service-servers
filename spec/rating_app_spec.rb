...
describe "Rating App" do
  ...
  describe "GET on /ratings/record/:record" do
    before(:each) do
      @rating = Fabricate(:rating)
    ...
    it "should return a score and count for a rating record" do
      get "/ratings/record/#{@rating.record}"
      last_response.should be_ok
      attributes = MessagePack.unpack(last_response.body)
  ...
  describe "POST on /ratings" do
    it "should create a rating" do
      ...
      post "/ratings", rating.to_msgpack
      last_response.should be_ok
      attributes = MessagePack.unpack(last_response.body)
      get "/ratings/#{attributes["id"]}"
      attributes = MessagePack.unpack(last_response.body)
  ...
  describe "PUT on /ratings/:id" do
    ...
    it "should update a rating" do
      ...
      put "/ratings/#{@id}", rating.to_msgpack
      last_response.should be_ok
      attributes = MessagePack.unpack(last_response.body)
      attributes["score"].should == rating[:score]
    end
  ...
  describe "DELETE on /ratings/:id" do
    before(:each) do
      @rating = Fabricate(:rating)
      @id = @rating.id.to_s
    end
